...
namespace HCA.Foundation.Commerce.Services.Wishlist
{
    ...
    public class WishlistService : IWishlistService
    {
        ...
        private readonly IWishlistConverter wishlistConverter;

        private readonly IWishlistManager wishlistManager;

        public WishlistService(
            IWishlistConverter wishlistConverter,
            IStorefrontContext storefrontContext,
            IVisitorContext visitorContext,
            IWishlistManager wishlistManager,
            ICatalogMapper catalogMapper)
        {
            Assert.ArgumentNotNull(wishlistConverter, nameof(wishlistConverter));
        ...
        public Result<Wishlist> AddWishlistLine(Variant product)
        {
            return this.ExecuteWithWishlist(
                (result, wishlist) =>
                {
                    ...
                    {
                        result.SetResult(this.wishlistConverter.Convert(addLinesToWishListResult.WishList));
                    }
                    else
                    {
        ...
        public Result<Wishlist> GetWishlist()
        {
            return this.ExecuteWithWishlist(
                (result, wishlist) =>
                {
                    result.SetResult(this.wishlistConverter.Convert(wishlist));
                    return result;
                });
        }
        ...
        public Result<Wishlist> RemoveWishlistLine(string variantId)
        {
            return this.ExecuteWithWishlist(
                (result, wishlist) =>
                {
                    ...
                    {
                        result.SetResult(this.wishlistConverter.Convert(removeWishListLinesResult.WishList));
                    }
                    else
                    {

 ...
 */
const bundlePath = process.env.SITECORE_JSS_SERVER_BUNDLE || `./dist/${appName}/project/${appName}/server.bundle`;

const serverBundle = require(bundlePath);

httpAgents.setUpDefaultAgents(serverBundle);

const apiHost = process.env.SITECORE_API_HOST || 'http://hca.local'

appName = appName || serverBundle.appName;

const apiKey = process.env.SITECORE_API_KEY || serverBundle.apiKey || ' {9DFF2B2D-206B-436E-B758-B716728B0F0D}';

...
1.  Deploy the build artifacts from your app (`/dist` or `/build` within the app) to the `sitecoreDistPath` set in your app's `package.json` under the proxy root path. The HCA app uses `/dist/${jssAppName}/Project/${jssAppName}`, for example `$proxyRoot/dist/HCA/project/HCA`.

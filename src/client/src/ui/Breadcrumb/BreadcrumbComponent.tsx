...
import { PageLink, BreadcrumbProps } from './models';
...
const BreadcrumbComponent = (props: BreadcrumbProps) => {
  ...
  const { breadcrumb } = props.sitecoreContext;
      ...
      {pageLinks &&
        pageLinks.map((pageLink: PageLink, index: number, array: PageLink[]) => (
          <span key={pageLink.title} className={cnBreadcrumbs('LinkItem')}>
            <Link aria-current="page" className={cnBreadcrumbs('Link')} to={pageLink.link || pathname}>
              {pageLink.title}
            </Link>

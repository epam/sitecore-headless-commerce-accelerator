...
namespace AutoTests.HCA.Core.UI.Pages
{
    public class ProductPage : CommonPage
    {
        ...
        private readonly WebElement _descriptionFeatures =
            new WebElement("Description Features", ByCustom.ClassName("description-features"));

        private readonly WebElement _descriptionRating =
            new WebElement("Description Rating", ByCustom.ClassName("description-rating"));

        private readonly WebElement _descriptionText =
            new WebElement("Product Description", ByCustom.ClassName("description-text"));
        ...
        private readonly WebElement _status =
            new WebElement("In Stock Status", ByCustom.ClassName("product-stock-status"));

        private readonly WebElement _title = new WebElement("Product Title", ByCustom.ClassName("product-title"));

        private readonly WebLink _writeReviewLink =
            new WebLink("Review Write Link", ByCustom.ClassName("review-write"));
        ...
        public decimal Price =>
            decimal.Parse(string.Join("", _price.GetText().Where(x => char.IsDigit(x) || x == '.')));
        ...
        public bool PrintButtonIsClickable()
        {
            return _printButton.IsClickable();
        }

        public bool WriteReviewIsClickable()
        {
            return _writeReviewLink.IsClickable();
        }

        public bool ReadReviewIsClickable()
        {
            return _readReviewLink.IsClickable();
        }

        public override string GetPath()
        {
            return PagePrefix.Product.GetPrefix();
        }

      ...
      <ul className="cart">
        {cartLines &&
          cartLines.map((item, index) => (
            <li key={index}>
              <figure>
                {item.variant.imageUrls ? (<img src={item.variant.imageUrls[0]} />) : (item.product.imageUrls && (<img src={item.product.imageUrls[0]} />))}
                <figcaption>
                  <Text field={{ value: item.product.brand }} tag="h4" />
                  <Text field={{ value: item.product.displayName }} tag="span" />
                  <br />
